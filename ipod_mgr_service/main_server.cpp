...
#endif
#if 0//def HAVE_LIBUSB_HOTPLUG_API
...
libusb_hotplug_callback_handle usb_hotplug_cb_handle;

static int usb_hotplug_cb(libusb_context *ctx, libusb_device *device, libusb_hotplug_event event, void *user_data)
{
	if (LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED == event) {
		usbmuxd_log(LL_INFO, "\e[31m%s() \e[0m  LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED  \n", __func__);
	} else if (LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT == event) {
		usbmuxd_log(LL_INFO, "\e[31m%s() \e[0m  LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT  \n", __func__);
	} else {
